...
describe("DiagramBuilder", () => {    
    ...
    ])(`Given model '$_model', when we request REMOVE without filter, then we get '$_expected'`, ({ _modelClass, _model, _expected }) => {
    ...
    ])(`Given model '$_model', when we request REMOVE '$_operation', then we get '$_expected'`, ({ _modelClass, _model, _operation, _direction, _relationType, _expected }) => {
        let diagramBuilder: DiagramBuilder = new DiagramBuilder(new ModelBuilder(_model).build(), OutputFormatType.PlantUml);
    ...
    ])(`Given model '$_model' for ADD, when we request ADD '$_operation', then we get '$_expected'`, ({ _modelClass, _model, _operation, _direction, _relationType, _expected }) => {
        let diagramBuilder: DiagramBuilder = new DiagramBuilder(new ModelBuilder(_model).build(), OutputFormatType.PlantUml);
        let diagram: string = diagramBuilder.setClass(_modelClass, ActionType.ADD)
            .coupling(_direction, _relationType)
            .build();
        diagram = new Strings().clearSpaces(diagram);
        console.log(diagram);
